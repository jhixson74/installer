...
func getTerraformPluginInfo(file *modfile.File) map[string]*TFPluginInfo {
	plugins := make(map[string]*TFPluginInfo)

	// terraform provider and name regex
	re := regexp.MustCompile(`^.+/(terraform-provider-([^/]+))(/v\d+)?$`)
...
func getTerraformPluginVersions(file *modfile.File, plugins map[string]*TFPluginInfo) map[string]*TFPluginInfo {
...
func getLocalTerraformPluginVersions(file *modfile.File, plugins map[string]*TFPluginInfo) map[string]*TFPluginInfo {
...
func main() {
	...
	plugins := getTerraformPluginInfo(file)

	// Get the versions for all of the terraform plugins and update our map
	plugins = getTerraformPluginVersions(file, plugins)

	// Get local plugin versions
	plugins = getLocalTerraformPluginVersions(file, plugins)
